...
namespace MyWay2021.Server.Controllers.Colaboradores
{
    ...
    public class AccaoFormacaoController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetListagemMyWay()
        {
            ...
            {
                count++;
                Debug.WriteLine("ID: " + colaborador.ColaboradorID);
                Debug.WriteLine("Número de colaboradores: " + count);
                ...
                var listItem = new ListagemAccoesMyWay
                {
                    ...
                    UnidadeHandling = colaborador.Uh.Nome,
                    Ativo = colaborador.Ativo,
                    NumPw = colaborador.NumPw,
                    Formando = colaborador.Nome,
                    SecurityInit = sec.Init ?? null,
                    SecurityEnd = sec.End ?? null,
                    DGRInit = dgr.Init,
                    DGREnd = dgr.End,
                    BPMRInit = bpmr.Init,
                    BPMREnd = bpmr.End,
        ...
        public InitEnd GetInitEnd(Guid colaboradorID, List<Guid> idsCursos)
        {
            AccaoFormacao accao = _db.AccoesFormacao
                .Where(c => c.ColaboradorID == colaboradorID && idsCursos.Contains(c.CursoID))
